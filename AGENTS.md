...
- Define fallible operations as Effects at their point of definition and carry expected failures through the typed error channel. Do not create helpers that throw and then recover by wrapping their callers in `try`/`catch` or `Effect.try`; reserve `Effect.try` and `Effect.tryPromise` for true boundaries to third-party or platform APIs that may throw or reject.
...
- Do not replace implementation-detail suffixes such as `Effect` or `Object` with a generic `Program` suffix. When two operations need distinct names, name their domain roles or behaviors. Preserve a technology term when it names the subject of an operation, as in `runWorkspaceEffect`; remove it only when it merely exposes how an otherwise domain-named operation is implemented.
...
- When addressing PR review feedback, reply to each addressed thread with a concise summary of the change and mark the thread resolved after the fix is pushed and validated.
...
- Workspace sales calendar events reference stored discount definitions: the trimmed description is exactly one discount UUID, Calendar owns occurrence timing, and Postgres owns the public label, adjustment, and product targets. Do not reintroduce TOML sale configuration.
