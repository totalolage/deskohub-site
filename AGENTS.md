...
- Preview environments and preview databases must never contain real PII; use synthetic data only. The one exception is Better Auth login email, pending-verification email, and short-lived rate-limit IP keys that an ordinary protected production-derived Neon branch may inherit from production. Preview fixtures and all Dotypos profile and reservation data stay synthetic; do not add a sanitizer or an alternate branch procedure.
...
- Treat codebase-index results as discovery and evidence. Repository instructions remain authoritative; load the matching skill and read any reference it requires before acting.
- Before using the codebase index, check `index_status`. Run incremental `index_codebase` when the index is missing or stale, and run `index_health_check` for reported stale or orphaned data. Retry `INDEX_BUSY` later. Reserve `force: true` for a full rebuild the user explicitly requests.
