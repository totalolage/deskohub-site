...
Run Workspace Nexi checkout E2E against the ordinary protected Vercel Git preview for the exact committed and pushed SHA. CI takes the immutable deployment URL from Vercel's successful deployment event and uses that single origin for browser traffic, result/status URLs, notification URLs, and webhook replays. Uncommitted local code has no externally reachable Git preview and is not tested through an older deployment. Configure the Workspace Preview environment with the Nexi sandbox origin and key; callbacks do not require a stable manually moved domain.
