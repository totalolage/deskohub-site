...
export const renderPostHogFeatureFlagContract = (
  definitions: readonly PostHogFeatureFlagDefinition[]
) =>
  Effect.gen(function* () {
    ...
    return [
      ...
      ...sortedDefinitions.map(
        (definition) => `  ${JSON.stringify(definition.key)},`
      ),
      "] as const;",
      "",
      ...
      ...sortedDefinitions.flatMap((definition) => [
        `  readonly ${JSON.stringify(definition.key)}: {`,
        `    readonly value: ${renderValueType(definition)};`,
        `    readonly payload: ${renderPayloadTypeForFlag(definition)};`,
        "  };",
      ]),
      "}",
      "",
