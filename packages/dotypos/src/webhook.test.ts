...
describe("parseDotyposWebhookPayload", () => {
  ...
  test("rejects non-array payloads", async () => {
    const result = await Effect.runPromise(
      parseDotyposWebhookPayload({ reservationid: 321 }).pipe(Effect.result)
    );

    expect(result._tag).toBe("Failure");
    if (result._tag === "Failure") {
      expect(result.failure.message).toBe("Invalid Dotypos webhook payload");
      expect("issues" in result.failure).toBe(false);
    }
  });
});
...
describe("verifyDotyposWebhookRequest", () => {
  ...
  test("verifies the secret and parses reservation records", async () => {
    const payload = await Effect.runPromise(
      verifyDotyposWebhookRequest(request(JSON.stringify([reservationRecord])), {
        requireSecret: true,
