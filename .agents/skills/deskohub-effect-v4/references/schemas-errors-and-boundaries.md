...
- In Workspace production code, carry Effects to `WorkspaceEffect.page`, `route`, `action`, or `task`; reserve the standalone `WorkspaceEffect.run` composition for scripts. Feature helpers and `.server.ts` modules are not execution boundaries.
- Keep Workspace censorship focused on redaction and censored logger construction. Do not add Effect execution, Next request access, or telemetry lifecycle scheduling to the censorship module.
