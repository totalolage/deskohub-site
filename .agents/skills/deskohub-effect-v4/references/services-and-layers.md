...
## Adapt Next Route Handlers

Wrap Effect-based Next.js Route Handlers with `NextEffect.route` and configure an application-owned request runner. Provide Layers at that shared application boundary instead of manually provisioning and executing Effects in individual Route Handlers. When a Route Handler maps failures to responses, apply that recovery in the application runner around the fully provided Effect so Layer acquisition failures follow the same mapping. Model the recovery response through the `NextEffectRouteRunner` and runtime type parameter instead of casting the runner or duplicating the runtime interface. Make request cancellation an explicit adapter policy so enabling it for one handler does not silently change webhook, cron, queue, payment, or other request lifecycles.
