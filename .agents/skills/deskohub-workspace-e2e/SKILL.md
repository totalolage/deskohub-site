...
- For a long-running `agent-browser eval`, set both the child-process timeout and `AGENT_BROWSER_DEFAULT_TIMEOUT` from the checked-in timeout for that operation. The browser's internal CDP deadline can otherwise terminate evaluation before the in-page script's own readiness deadline.
