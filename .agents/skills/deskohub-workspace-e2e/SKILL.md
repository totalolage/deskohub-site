...
- Before attaching deployment-protection authorization to a browser-derived navigation, validate HTTPS, the exact expected preview host, and the expected localized application path. Fail closed on an off-origin or path-confused target. Structurally redact sealed capability query parameters from command, output, error, and artifact logging; registering only known environment-derived values is insufficient for dynamically issued capabilities.
...
- Seed source-neutral discount definitions and codes only in the exact preview database before parallel cases start. Keep the dedicated long-lived Calendar event immutable. When a pricing-change case must mutate its stored definition, isolate it on a product identity unused by happy paths, serialize the related mutations inside one top-level case, and restore the target with an interruption-safe finalizer. Never mutate a target consumed by another parallel case.
