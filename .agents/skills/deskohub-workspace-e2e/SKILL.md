...
- Do not await a long UI- or provider-backed preparation promise inside one `agent-browser eval`: agent-browser's CDP `Runtime.evaluate` response can time out before the semantic E2E deadline. Start only side-effect-free preparation in a short synchronous evaluation, retain its bounded status in the page, poll that status with short evaluations under the existing semantic timeout, and perform the state-creating native activation only after preparation succeeds. Preserve preparation errors and do not increase timeouts or retry checkout/payment creation to hide a CDP transport limit.
