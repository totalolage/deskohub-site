...
- Do not add one-off environment switches to enable database integration checks. Name Bun cases `*.e2e.test.ts`, exclude that pattern from the default unit-test command, run them through an explicit `test:e2e` command, and use the typed E2E environment plus the preview-database allowlist as the safety boundary.
