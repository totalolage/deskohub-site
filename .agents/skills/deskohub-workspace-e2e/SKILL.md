...
- Run independent E2E cases with the checked-in bounded fail-fast Effect concurrency. Do not make the aggregate unbounded: the combined browser, preview, Dotypos, and Nexi load can saturate the preview and turn valid pay pages into generic checkout failures. Do not convert case effects to `Exit` before the parallel aggregate; doing so makes failures look successful to the parent and prevents sibling interruption.
...
- Keep one daemon-wide `AGENT_BROWSER_DEFAULT_TIMEOUT`, derived from the checked-in checkout-start budget, on every `agent-browser` child command, and set the focused child-process timeout separately. Changing the browser environment only for a long-running `eval` can reset the shared daemon and lose the already-open sessions; leaving its default untouched can terminate CDP evaluation before the in-page readiness deadline.
