...
- For tooltip assertions, wait for the trigger's focus handler, center it
  instantly in the viewport, and wait for layout to settle. A programmatic
  focus command can succeed without opening a Radix tooltip, and a pointer
  hover does not reliably survive separate CLI commands. Use focus only to
  anchor the target, then exercise Radix's native keyboard contract by pressing
  `Shift+Tab` and `Tab` before reading the overlay. Do not paper over a missing
  interaction by increasing the overlay wait.
