...
Match worker markers from comments to the current open-issue list by issue number before checking threads. Repository-wide comments include closed issues. Both worker helpers verify the issue is open before dispatch; exit 3 means skip it. On a GitHub lookup failure, leave it for a later pass.
