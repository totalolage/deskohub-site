...
Treat a goods-order write-off as an explicit idempotent collection decision, not cancellation, payment, refund, or fulfillment. Permit it only for fulfilled unpaid goods with no live payment attempt, preserve every lifecycle state, and expose only its timestamp through safe order projections.
