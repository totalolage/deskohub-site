...
During reservation-HMAC migration, keep the database-enforced identity digest derived from dedicated reservation HMAC material separate from the raw AES key-ring compatibility digest. Current writers must persist both uniqueness planes so real old writers collide in either insert order, and each request must freeze its HMAC generation once at operation entry across retries and checkout-session rotation. Do not retire or change the raw compatibility write until every old deployment, rollback target, queued job, and in-flight operation is drained.
