...
Keep payment-tab operations inside the hosted-provider result branch. Once a provider session exists, expose its hosted page through a one-shot user-activated new-tab link. From that click, synchronously create and retain the exact script-owned payment window on the original browser window, not in route-module state, before navigating the original tab to status. The original status tab must close that tracked window once it returns to the expected same-origin status path; keep authenticated Web Locks as the cross-tab fallback and retain the native link as the popup-blocked fallback. Internal zero-total completion, pricing changes, validation failures, and any other result that creates no provider session must not open or manage a browser tab.
