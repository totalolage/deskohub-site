...
Consume a different-provider recovery's send-capable phase immediately before its non-idempotent cancellation boundary. Pre-boundary failures may release to send-capable recovery; after consumption, failure, defect, interruption, stale visibility, or incomplete cardinality proof may release only to exact awaiting-visibility recovery. Redelivery may reverify that phase but must never send cancellation again, and final cleanup still requires definitive loser `CANCELLED` status plus the full winner/cardinality proof.
...
Preserve an attachment-compensation row's exact provider-creation epoch through cancellation claim and failure. A delayed message may restore legacy generic cancellation failure only when the row remains retryable attachment-compensation work with a recognized cancellation failure code; it must never replace a different exact epoch or revive `manual_review`. Treat `reservation_hold_expired_at` as immutable first-observation evidence: database-clock claim retries and repeated pending-payment cleanup skips may populate it only when null.
