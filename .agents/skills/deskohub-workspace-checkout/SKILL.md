...
Keep payment-tab operations inside the hosted-provider result branch. Once a provider session exists, expose its hosted page through a one-shot user-activated new-tab link. From that click, synchronously create the script-owned payment window and write authenticated tab-local ownership to both tabs before navigating either one. The original status tab's authenticated BroadcastChannel heartbeat closes only the matching returned payment tab; retain the scoped Web Lock as an independent ownership fallback and the native link as the popup-blocked fallback. Internal zero-total completion, pricing changes, validation failures, and any other result that creates no provider session must not open or manage a browser tab.
