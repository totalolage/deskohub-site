...
Do not time-gate PIN disclosure or clear an issued PIN according to the application clock. Once an authorized paid, locally confirmed, live-confirmed reservation has an issued PIN, show it on the protected access page. Igloohome's programmed AlgoPIN bounds alone determine when the lock accepts it; locally stored bounds exist only for issuance and schedule-change recovery. When a live reservation move changes the rounded provider interval, automatically replace a known issued PIN for the new interval; reserve manual reconciliation for ambiguous provider outcomes.
