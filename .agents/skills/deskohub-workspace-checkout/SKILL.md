...
Keep payment-tab operations inside the hosted-provider result branch. Once a provider session exists, expose its hosted page through a one-shot user-activated new-tab link within the existing order-summary page; do not replace checkout with a second summary state. Lock prior summary inputs while preserving their visible context. From the provider link click, synchronously create the script-owned payment window, mark the original status tab as owner, and navigate both tabs. Preserve the established scoped Web Lock ownership and closure flow; do not replace it with new cross-tab coordination. Retain the native link as the popup-blocked fallback. Internal zero-total completion, pricing changes, validation failures, and any other result that creates no provider session must not open or manage a browser tab.
