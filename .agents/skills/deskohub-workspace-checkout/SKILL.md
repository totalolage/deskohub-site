...
During reservation-HMAC migration, preserve three database uniqueness planes: the established columns use the request's scheduled current digest, dedicated identity columns always use reservation-HMAC material, and raw compatibility columns always use the unchanged AES key-ring bytes. The migrated database trigger must default both added planes from the established columns for genuine old inserts, and current readers must check all planes for divergent rows before applying the bounded read-window policy. Current writers must persist all three planes, repair an old winner's missing current-plane values before compare-and-set, and freeze HMAC generation once at operation entry across retries and checkout-session rotation. Do not retire or change the raw compatibility write until every old deployment, rollback target, queued job, and in-flight operation is drained.
