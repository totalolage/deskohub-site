...
Keep payment-tab operations inside the hosted-provider result branch. Once a provider session exists, expose its hosted page through a user-activated new-tab link. From that click, synchronously create a script-owned payment tab before navigating the original tab to status so the returned payment tab remains eligible for `window.close()`; retain the native link as the popup-blocked fallback. Internal zero-total completion, pricing changes, validation failures, and any other result that creates no provider session must not open or manage a browser tab.
