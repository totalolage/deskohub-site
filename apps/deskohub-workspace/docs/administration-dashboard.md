...
When a window still has an unreconciled ambiguous or stale attempt, a new creation for that exact device and window is refused with an explicit cleanup-required outcome before any provider call, regardless of remaining capacity. The refusal is discovered server-side, so it also applies after a page reload or a new CLI process. The outcome identifies the one prior attempt that the operator must inspect. The operator's confirmation names that exact attempt, records an append-only reconciliation event under the same device-and-window lock, and frees only that attempt. If another attempt still requires inspection, Workspace returns it as a new cleanup-required outcome instead of reconciling it implicitly. Replaying an ambiguous attempt identifier without confirmation still reports its ambiguous outcome, while confirming that same attempt records the reconciliation and reports a reconciled outcome that authorizes the CLI to issue a fresh attempt in the same invocation. The confirmation control appears in the administration form and as a `dhw` flag only after an ambiguous or cleanup-required result.
...
Operator cancellation is available for a held, confirmed, or retryable failed-cancellation reservation only while fulfillment is quiet: both an active confirmation run and a still-unresolved confirmation-email delivery block cancellation until they complete. A pending payment remains protected by default. A CLI operator who has reviewed the provider payment may explicitly force cancellation; Workspace then atomically marks the pending attempt cancelled and releases its reserved discount or voucher claim before cancelling the booking. A later provider settlement cannot recreate an operator-cancelled booking and is marked as requiring a refund. Cancellation otherwise preserves payment, fulfillment, discount, legal, and accounting history. Customer email delivery happens only after cancellation succeeds; an email failure is reported separately and never misrepresents the booking as active.
...
The Reservations count shown in the Customers table and on customer detail pages counts only successful reservations. A successful reservation is fulfilled and not cancelled, cancelling, or cancellation_failed. The Reservations sort uses that same count. Customer listing, paging, and last activity remain based on all reservation attempts, and visible reservation history remains complete.
