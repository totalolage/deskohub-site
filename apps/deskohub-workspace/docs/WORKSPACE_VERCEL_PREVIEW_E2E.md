...
The checkout E2E workflow provisions or reuses the dedicated `workspace-checkout-e2e` Neon branch and migrates it explicitly before deploying a fresh preview. For other preview or development databases, run `bun run db:migrate` before deploying whenever the branch contains pending Workspace migrations. The app's `build` script remains the atomic core Next build (`next build --turbo`), while Turbo's Workspace build task is responsible for running `i18n:compile` and upstream generation such as `@deskohub/nexi#generate` before that build starts.
