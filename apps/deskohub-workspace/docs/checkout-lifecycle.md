...
The narrow access exception stores an issued, time-bound AlgoPIN in a dedicated credential ledger so repeated access-page requests reuse the same code without creating another one. Workspace retains the PIN and does not use its own clock to expire or hide it; Igloohome enforces its programmed validity at the lock. A reservation timing change after issuance withholds the old PIN for operator reconciliation. Secret codes remain excluded from email, logs, analytics, ordinary reservation records, and operator-facing payloads; safe grant state, interval, device, provider identifier, and failure metadata may be shown to operators.
