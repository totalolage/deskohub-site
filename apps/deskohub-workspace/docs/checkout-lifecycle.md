...
The protected access page resolves the current PIN on each authorized request for a paid reservation that is confirmed both locally and by the reservation provider. It does not impose a separate display window. The time bounds programmed into the AlgoPIN are the sole authority for when the lock accepts it.
