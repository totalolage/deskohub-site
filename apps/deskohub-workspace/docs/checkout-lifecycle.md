...
A payment that settles after Workspace has expired its local attempt starts durable recovery. Workspace reuses the original hold when cancellation has not taken effect; otherwise it recreates the exact accepted reservation only when the interval has not ended, no newer checkout reservation exists, and current capacity is available. Successful recovery continues normal paid fulfillment. An unavailable, superseded, or unreconstructable reservation requires a refund, while an ambiguous provider state requires operator review.
