...
The narrow access exception stores an issued, time-bound algoPIN in a dedicated credential ledger so repeated access-page requests reuse the same code without creating another one. The daily cleanup removes expired PIN values. A reservation timing change after issuance withholds the old PIN for operator reconciliation. Secret codes remain excluded from email, logs, analytics, ordinary reservation records, and operator-facing payloads; safe grant state, interval, device, provider identifier, and failure metadata may be shown to operators.
