...
The dedicated sales calendar owns only when a sale is active. The referenced `discounts` row owns the complete customer-facing label map, adjustment, and product targets. Checkout resolves exactly `labels[locale]` after loading the definition; there is no cross-language fallback. Missing, blank, or unsupported translations make that definition unavailable for the resolution, without suppressing other valid discounts.
...
Cancelled events and events without a description are ignored. Any non-empty description that is not exactly one valid UUID is an operational configuration error: log it at Error level and omit only that event. A missing, malformed, or temporarily unavailable referenced definition is also logged and omitted independently, so other valid events and discount sources still resolve. A whole Calendar outage produces no Calendar candidates without blocking checkout. Partial results are not cached as complete successes. Cancel or delete the event to stop the sale; do not delete a definition while an active production event references it.
...
`countdownStartsAt <= now < expiresAt`. It counts remaining partial hours up
until the final hour and remaining partial minutes thereafter. Calendar sales
declare a 24-hour countdown window; bounded codes declare a one-hour window.
The UI does not infer either source.
