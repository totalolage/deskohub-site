...
export const provisionWorkspaceE2ECoordination = Effect.gen(function* () {
  ...
  if (!currentDatabase) {
    return yield* Effect.fail(
      new CoordinationProvisioningError({
        message: "Could not identify the Workspace E2E coordination database.",
      })
    );
  }
...
const program = Schema.decodeUnknownEffect(Environment)(process.env).pipe(
  Effect.flatMap((environment) =>
    provisionWorkspaceE2ECoordination.pipe(
      Effect.provide(
        PgClient.layer({
