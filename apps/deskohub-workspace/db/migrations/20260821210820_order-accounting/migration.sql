...
ALTER TABLE "invoices" DROP CONSTRAINT "invoices_source_reference_check", ADD CONSTRAINT "invoices_source_reference_check" CHECK (("payment_attempt_id" is null and "order_id" is null and "workspace_reservation_id" is null) or ("payment_attempt_id" is not null and ("order_id" is not null or "workspace_reservation_id" is not null)));--> statement-breakpoint
...
BEGIN
	...
	IF NOT EXISTS (
		...
		RAISE EXCEPTION 'invoice source must be manual or the active paid attempt and snapshot of the fulfilled order'
			USING ERRCODE = '23514';
	END IF;
