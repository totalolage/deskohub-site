...
export const createWorkspaceEffectRunner = (
  loggerProvider?: LoggerProvider
) => {
  const loggerLive = createWorkspaceLoggerLive(loggerProvider);
  const runWithLogAnnotations = <A, E>(
  ...
  return {
    run: <A, E>(effect: Effect.Effect<A, E, never>) =>
      runWithLogAnnotations(effect, {}),
    runWithLogAnnotations,
  } as const;
};
