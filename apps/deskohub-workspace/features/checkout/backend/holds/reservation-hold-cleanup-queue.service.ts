...
import { getS5CancellationAction } from "@/features/reservation/backend/s5-cancellation-policy.compat";
import {
  ...
  return {
    ...
    options: {
      ...
      idempotencyKey: input.stabilizeCandidate
        ? `reservation-provider-candidate-stabilization:${input.orderId}:${input.providerCreationEpoch}:${dotyposReservationId}`
...
export const enqueueAttachmentCancellationCompensation = Effect.fn(
  ...
  const normalizeMarker = <E>(effect: Effect.Effect<void, E>) =>
    effect.pipe(
      Effect.mapError(
        () =>
...
const recoverUnattachedProviderHold = Effect.fn(
  ...
  const outcome = yield* cleanup
    .cancelOrderHold({ orderId: input.orderId })
    .pipe(
      Effect.mapError(
        ReservationHoldCleanupScheduleError.fromError(
...
const verifyExactProviderWinnerEvidence = Effect.fn(
  ...
  const allEvidenceIsDefinitive = matches.every((reservation) => {
    ...
    return (
      validIdentity &&
      (reservation === winner ||
        getS5CancellationAction(reservation.status) === "complete")
    );
  });
...
const verifyReleasedProviderCancellation = Effect.fn(
  ...
  if (
    ...
    getS5CancellationAction(candidate.status) !== "complete"
  ) {
    return yield* new ReservationHoldCleanupScheduleError({
...
export const processReservationHoldCleanupScheduleMessage = Effect.fn(
  ...
  if (
    ...
    const ownedRecovery = Effect.gen(function* () {
      ...
      const action = getS5CancellationAction(candidate.status);
      if (action === "refuse") {
        return yield* failRecovery(
  ...
  if (payload.schemaVersion !== 1) return "ignored" as const;
