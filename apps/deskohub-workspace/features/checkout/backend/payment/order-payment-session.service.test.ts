...
describe("OrderPaymentSessionService", () => {
  ...
  test("locks a reservation before its canonical order during admission", async () => {
    ...
    const ownerValidatorStart = source.indexOf(
      "const validatePaymentSessionSnapshotOwner = Effect.fn("
    );
    const ownerValidatorEnd = source.indexOf(
      "const lockReservationForAdmission",
      ownerValidatorStart
    );
    const ownerValidator = source.slice(ownerValidatorStart, ownerValidatorEnd);

    expect(admission).toContain(
      'input.evidence.mode === "reservation_attempt_commitment"'
    );
    expect(admission).toContain("reservation: lockedReservation");
    ...
    expect(ownerValidator).toContain(
      "input.reservation?.dotyposReservationId !=="
    );
    expect(ownerValidator).toContain("input.snapshot.dotyposReservationId");
  });
});
