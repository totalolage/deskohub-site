...
import { loadAdministrationNexiOperation } from "@/features/administration/page-data.server";
import {
...
export default function NexiOperationAdministrationDetailPage({
  params,
}: {
  ...
  return (
    <AdministrationPage>
      ...
      >
        <NexiOperationAdministrationDetail params={params} />
      </Suspense>
    </AdministrationPage>
  );
}

export async function NexiOperationAdministrationDetail({
  params,
}: {
  readonly params: Promise<{ readonly operationId: string }>;
}) {
  const { operationId } = await params;
  const detail = await loadAdministrationNexiOperation(operationId);
        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_20rem]">
          ...
          <aside className="space-y-5 xl:sticky xl:top-24 xl:h-fit">
            <RelatedEntity
              href={
                operation.orderId
                  ? `/admin/nexi/orders/${encodeURIComponent(operation.orderId)}`
                  : undefined
              }
