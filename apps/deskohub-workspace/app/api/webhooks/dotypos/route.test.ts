...
describe("Dotypos webhook route", () => {
  ...
  test("invalid payload returns 400 without invalidation", async () => {
    ...
    const invalidJsonResponse = await POST(request("not json"));
    ...
    const invalidShapeResponse = await POST(
      jsonRequest({ reservationid: "raw-payload-value" })
    );
    const invalidShapeBody = await invalidShapeResponse.json();

    expect(invalidShapeResponse.status).toBe(400);
    expect(invalidShapeBody).toEqual({
      error: "Invalid payload",
