...
import { processReservationHoldCleanupQueueMessage } from "@/features/checkout/backend/reservation-hold-cleanup-queue.service";
import { WorkspaceReservationRepositoryLive } from "@/features/reservation/backend/workspace-reservation.repository";
import { runWorkspaceEffect } from "@/shared/backend/logging/censorship";

const ReservationHoldCleanupQueueConsumerLive = Layer.mergeAll(
...
export const POST = handleCallback(async (message) => {
  await runWorkspaceEffect(
    processReservationHoldCleanupQueueMessage(message).pipe(
      Effect.provide(ReservationHoldCleanupQueueConsumerLive),
      Effect.annotateLogs({ operation: "reservationHoldCleanupQueue" })
    )
  );
});
