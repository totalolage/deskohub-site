...
describe("goods orders route", () => {
  ...
  test("issues only acknowledged server-affirmed facts", async () => {
    ...
    const { POST } = makeGoodsOrdersRoutes(
      Layer.mergeAll(
        ...
        Layer.mock(GoodsOrderService, {
          findByIssuanceRequest: () => Effect.succeed(Option.none()),
          issue: (input) => {
  ...
  test("replays an issued order without reopening a stale quote", async () => {
    ...
    const { POST } = makeGoodsOrdersRoutes(
      Layer.mergeAll(
        ...
        Layer.mock(GoodsOrderService, {
          findByIssuanceRequest: () => Effect.succeed(Option.some(detail)),
          issue: () => {
            issueCalls += 1;
            return Effect.succeed(detail);
          },
  ...
  test("rejects a reused issuance identifier with a different quote token", async () => {
    const { POST } = makeGoodsOrdersRoutes(
      Layer.mergeAll(
        Layer.mock(CustomerAccountResolver, {
          resolve: () => Effect.succeed(account),
        ...
        Layer.mock(GoodsQuoteService, {}),
        Layer.mock(GoodsOrderService, {
          findByIssuanceRequest: (_customerId, request) =>
            request.quoteToken === "sealed-token"
              ? Effect.succeed(Option.some(detail))
              : Effect.fail(
                  new GoodsOrderIssuanceConflictError({ message: "conflict" })
                ),
          issue: () =>
            Effect.fail(
              new GoodsOrderIssuanceConflictError({ message: "conflict" })
    ...
    const response = await POST(
      jsonRequest({
        issuanceId: "018f1e36-7a31-7c07-90f4-8f2531cd1212",
        quoteToken: "different-sealed-token",
        acknowledged: true,
      })
    );

    expect(response.status).toBe(409);
    expect(await response.json()).toEqual({ error: "issuance_conflict" });
  });
