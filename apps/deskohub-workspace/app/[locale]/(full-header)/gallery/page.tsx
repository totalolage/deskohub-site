...
export default async function GalleryPage({ params }: GalleryPageProps) {
  ...
  return runWithRequestLocale(locale, async () => {
    ...
    if (result.status === "error") {
      return <Gallery isUnavailable photos={[]} />;
    }

    return (
      <Gallery
        assetsCount={result.assets.length}
        photos={toGalleryPhotos(result.assets)}
      />
    );
  });
}
...
function Gallery({
  ...
  photos: ReturnType<typeof toGalleryPhotos>;
}) {
  const hasPhotos = photos.length > 0;

  return (
    <main className="mt-[var(--site-header-height)] min-h-dvh overflow-x-clip bg-[#f4f1ea] text-navy-blue">
      ...
      <section className="relative isolate py-6 sm:py-8 lg:py-10">
        ...
        <Container className="max-w-7xl">
          {hasPhotos ? (
            ...
            <EmptyGallery assetsCount={assetsCount} />
          )}
        </Container>
      </section>
    </main>
