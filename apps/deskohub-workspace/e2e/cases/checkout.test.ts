  ...
  const run = (async (_command: string, args: string[]) => {
    ...
    return {
      ...
      stdout: readsUrl
        ...
        : "Your reservation is confirmed. Your payment is complete and the secure reservation link has been sent by email. Tuesday, August 25, 2026 10:00\u202fAM\u2009–\u20092:00\u202fPM CZK\u00a00",
    };
  }) as Runner;
...
test("asserts office range, seats, and price on the fulfilled status page", async () => {
  ...
  const run = (async (_command: string, args: string[]) => {
    ...
    return {
      ...
      stdout: readsUrl
        ? "https://workspace.test/en-US/reservation/status/order-id?outcome=success"
        : `Your reservation is confirmed. Your payment is complete and the secure reservation link has been sent by email. Private office ${expectedDateRange} SEATS 2 ${expectedPrice}`,
    };
  }) as Runner;
